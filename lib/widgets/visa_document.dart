...
class VisaDocument extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (hasValidParentData) {
      return _buildVisaContent(context, userName, photoUrl, gender ?? 'X', age ?? 0);
    }
    ...
    return FutureBuilder<Map<String, dynamic>>(
      ...
      builder: (context, snapshot) {
        ...
        return _buildVisaContent(context, finalName, finalPhoto, finalGender, finalAge);
      },
    );
  }

  // 🎨 THE ACTUAL UI PAINTER
  Widget _buildVisaContent(BuildContext context, String name, String? photo, String sex, int ageVal) {
      ...
      child: FittedBox(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(color: Colors.black.withOpacity(0.2), offset: const Offset(2, 2), blurRadius: 0),
              BoxShadow(color: Colors.black.withOpacity(0.15), offset: const Offset(0, 8), blurRadius: 16),
              BoxShadow(color: theme.primaryColor.withOpacity(0.05), blurRadius: 20, spreadRadius: 2)
            ],
          ),
          ...
          child: Stack(
            children: [
              // PAINTERS LAYER
              Positioned.fill(child: CustomPaint(painter: UniversalGuillochePainter(color: theme.primaryColor.withOpacity(0.08), secondaryColor: theme.secondaryColor.withOpacity(0.06), batch: theme.batch))),
              Positioned(right: -30, top: 10, child: Opacity(opacity: 0.08, child: SizedBox(width: 260, height: 260, child: CustomPaint(painter: BatchSymbolPainter(batch: theme.batch, color: theme.primaryColor))))),
              Positioned.fill(child: ClipRRect(borderRadius: BorderRadius.circular(12), child: BackdropFilter(filter: ui.ImageFilter.blur(sigmaX: 1.5, sigmaY: 1.5), child: Container(color: Colors.white.withOpacity(0.10))))),
                ...
                child: Column(
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Text(cuisine.toUpperCase(), style: TextStyle(fontWeight: FontWeight.w900, fontSize: 10, letterSpacing: 1, color: theme.primaryColor)),
                        ),
                      ],
                    ),
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: hasPhoto
                                ? (photo!.startsWith('/')
                                    ? Image.file(
                                        ...
                                        errorBuilder: (context, error, stackTrace) {
                                          return Center(child: Icon(Icons.person, color: theme.primaryColor.withOpacity(0.2)));
                                        },
                                      )
                                    : Image.network(
                                        photo,
                                ...
                                : Center(child: Icon(Icons.person, color: theme.primaryColor.withOpacity(0.2))),
                          ),
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                _buildLabel("Surname / Nom", theme.primaryColor),
                                Text(safeName.split(' ').last.toUpperCase(), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, fontFamily: 'Courier', color: Colors.black)),
                                const SizedBox(height: 6),
                                _buildLabel("Given Names / Prénoms", theme.primaryColor),
                                Text(safeName.split(' ').first.toUpperCase(), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, fontFamily: 'Courier', color: Colors.black)),
                                const SizedBox(height: 6),
                                ...
                                Row(
                                  children: [
                                    ...
                                    const SizedBox(width: 20),
                                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildLabel("Issued", theme.primaryColor), Text(dateIssued, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 10, fontFamily: 'Courier', color: Colors.black))]),
                                  ],
                                ),
                              ],
                            ),
                    ...
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Text(mrzLine1, style: const TextStyle(fontFamily: 'Courier', fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: 1.5, height: 1.1, color: Colors.black54)),
                      Text(mrzLine2, style: const TextStyle(fontFamily: 'Courier', fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: 1.5, height: 1.1, color: Colors.black54)),
                    ]),
                  ],
                ),
              ),
  ...
  Widget _buildLabel(String text, Color color) {
    return Text(text.toUpperCase(), style: TextStyle(fontSize: 7, color: color.withOpacity(0.8), fontWeight: FontWeight.bold));
  }
}