...
class VisaTheme {
  ...
  VisaTheme(this.countryName, this.countryCode, this.primaryColor, this.secondaryColor, this.batch);
  ...
  static VisaTheme getTheme(String cuisine, Color fallbackColor) {
    ...
    if (lower == 'global' || lower == 'earth' || lower == 'world') {
      return VisaTheme("Planet Earth", "WLD", const Color(0xFF1A237E), const Color(0xFFFFD700), VisaBatch.global);
      // Deep Navy + Gold
    }
    
    // 1. SINOSPHERE (East Asia)
    if (_matches(lower, ['japan', 'sushi', 'china', 'dim sum', 'korea', 'kimchi', 'ramen', 'viet', 'pho', 'taiwan'])) {
       ...
       return VisaTheme("East Asia Territory", "EAS", primary, Colors.redAccent, VisaBatch.sinosphere);
    }

    // 2. OLD WORLD (Europe)
    if (_matches(lower, ['ital', 'pizza', 'pasta', 'franc', 'bistro', 'german', 'spanish', 'tapas', 'greek', 'uk', 'british'])) {
       ...
       return VisaTheme("Schengen Zone", "EUR", primary, const Color(0xFFCD212A), VisaBatch.oldWorld);
    }

    // 3. SUN & STONE (Latin America)
    if (_matches(lower, ['mexic', 'taco', 'peru', 'brazil', 'argentin', 'cuban', 'latin'])) {
       return VisaTheme("Americas South", "LAT", const Color(0xFF006847), const Color(0xFFCE1126), VisaBatch.sunAndStone);
    }

    // 4. ARABESQUE (Middle East)
    if (_matches(lower, ['lebano', 'falafel', 'turk', 'kebab', 'arab', 'middle east', 'persian'])) {
       return VisaTheme("Levant Region", "ME", const Color(0xFF007A3D), const Color(0xFFC8102E), VisaBatch.arabesque);
    }

    // 5. OCEANIC (Pacific)
    if (_matches(lower, ['australia', 'aussie', 'new zealand', 'hawaii', 'poke'])) {
       return VisaTheme("Oceania", "OCE", const Color(0xFF00008B), const Color(0xFFFFD700), VisaBatch.oceanic);
    }

    // 6. SUBCONTINENT (South Asia)
    if (_matches(lower, ['india', 'curry', 'pakistan', 'bengal', 'nepal'])) {
       return VisaTheme("South Asia", "IND", const Color(0xFFFF9933), const Color(0xFF138808), VisaBatch.subcontinent);
    }

    // 7. NEW WORLD (North America)
    if (_matches(lower, ['usa', 'american', 'burger', 'bbq', 'canada'])) {
       return VisaTheme("North America", "USA", const Color(0xFF3C3B6E), const Color(0xFFB22234), VisaBatch.newWorld);
    }
    
    // 8. TEXTILE (Africa)
    if (_matches(lower, ['ethiop', 'nigeria', 'african', 'morocco'])) {
      return VisaTheme("African Union", "AFR", const Color(0xFF009639), const Color(0xFFFFCE00), VisaBatch.textile);
    }

    // 9. NORDIC (Scandinavia)
    if (_matches(lower, ['swed', 'norway', 'danish', 'scandi'])) {
      return VisaTheme("Nordic Council", "NOR", const Color(0xFF006AA7), const Color(0xFFFECC00), VisaBatch.nordic);
    }

    // 10. CONSTRUCT (Eastern Bloc)
    if (_matches(lower, ['russia', 'polish', 'ukrain'])) {
      return VisaTheme("Eastern Bloc", "EUS", const Color(0xFFD52B1E), Colors.black, VisaBatch.construct);
    }
    ...
    return VisaTheme("Territory of $cuisine", cuisine.substring(0, 3).toUpperCase(), fallbackColor, Colors.grey, batch);
  }
...
class UniversalGuillochePainter extends CustomPainter {
  ...
  UniversalGuillochePainter({required this.color, required this.secondaryColor, required this.batch});

  @override
  void paint(Canvas canvas, Size size) {
    // 🖊️ Increased stroke width slightly for better visibility
    final paint = Paint()..style = PaintingStyle.stroke..strokeWidth = 0.5..isAntiAlias = true;
    ...
    switch (batch) {
      ...
      case VisaBatch.global: // 🌍 THE COMPASS ROSE (Diplomatic Style)
        ...
        for (int i = 0; i < 6; i++) {
           ...
           canvas.drawOval(Rect.fromCenter(center: Offset(cx, cy), width: radiusX * 2, height: 360), paint);
        }
        // Draw Parallels (Horizontal lines)
        for (int i = 0; i < 6; i++) {
           double radiusY = 180.0 * (i / 6);
           canvas.drawOval(Rect.fromCenter(center: Offset(cx, cy), width: 360, height: radiusY * 2), paint);
        }

        // 2. The Compass Rose (Center)
        paint.color = secondaryColor.withOpacity(0.3); // Gold
        _drawStar(canvas, cx, cy, 120, 4, 0.4, paint..strokeWidth = 1); // Main Points (N,E,S,W)
        _drawStar(canvas, cx, cy, 80, 4, 0.4, paint, rotation: math.pi/4); // Sub Points (NE, SE...)

        // 3. The Outer Ring
        paint.color = color.withOpacity(0.4);
        _drawSpirograph(canvas, cx, cy, 190, 185, 20, 100, paint..strokeWidth = 0.5);
        break;

      case VisaBatch.sinosphere: // 🌸 3D IMPERIAL
        canvas.save();
        final matrix = Matrix4.identity()..setEntry(3, 2, 0.0015)..rotateX(0.6)..rotateZ(0.1);
        canvas.translate(cx, cy); canvas.transform(matrix.storage); canvas.translate(-cx, -cy);
      ...
      case VisaBatch.nordic: // ❄️ 3D AURORA (High Vis)
        canvas.save();
        final auroraMatrix = Matrix4.identity()..setEntry(3, 2, 0.002)..rotateX(-0.6)..rotateY(0.1);
        canvas.translate(cx, cy); canvas.transform(auroraMatrix.storage); canvas.translate(-cx, -cy);
      ...
      case VisaBatch.subcontinent: // 🕉️ THE MAHARAJA'S FRAME (Vertical Rubies + Axis Emeralds)
        ...
        _drawSpirograph(canvas, size.width, size.height, cornerR, 30, 80, 12, paint);

        // LAYER 3: The "Connective Border"
        paint.color = color.withOpacity(0.4);
        for (double i = 0; i < size.width; i+=5) {
          canvas.drawCircle(Offset(i, 20 + 10 * math.sin(i/20)), 2, paint);
          canvas.drawCircle(Offset(i, size.height - 20 + 10 * math.sin(i/20)), 2, paint);
        }
        ...
        canvas.drawRect(Rect.fromLTWH(20, 20, size.width - 40, size.height - 40), rectPaint);
        canvas.drawRect(Rect.fromLTWH(24, 24, size.width - 48, size.height - 48), rectPaint);
        ...
        final List<Offset> corners = [
          Offset(55, 55), Offset(size.width - 55, 55),
          Offset(55, size.height - 55), Offset(size.width - 55, size.height - 55)
        ];
        ...
        final emeraldPaint = Paint()
          ..style = PaintingStyle.stroke
          ..strokeWidth = 0.6
          ..color = Colors.greenAccent[700]!.withOpacity(0.9); // Deep Emerald Green
        ...
        for (final pos in emeraldPos) {
          ...
          canvas.drawCircle(Offset.zero, 3, emeraldPaint..style = PaintingStyle.fill);
  ...
  void _drawStar(Canvas canvas, double cx, double cy, double radius, int points, double innerRatio, Paint paint, {double rotation = 0}) {
    final path = Path();
    final double step = math.pi / points;
    
    for (int i = 0; i < 2 * points; i++) {
      final double r = (i % 2 == 0) ? radius : radius * innerRatio;
      final double currAngle = i * step + rotation - (math.pi/2); // -pi/2 to start at top
  ...
  void _drawSpirograph(Canvas canvas, double cx, double cy, double R, double r, double offset, double revolutions, Paint paint) {
    final path = Path();
    final double step = (math.pi * 2) / 360; 
    bool firstPoint = true;
    for (double t = 0; t <= math.pi * 2 * revolutions; t += step) {
      final double x = cx + (R - r) * math.cos(t) + offset * math.cos(((R - r) / r) * t);
      final double y = cy + (R - r) * math.sin(t) - offset * math.sin(((R - r) / r) * t);
  ...
  @override
  bool shouldRepaint(covariant UniversalGuillochePainter old) => old.batch != batch || old.color != color;
}
...
class BatchSymbolPainter extends CustomPainter {
  ...
  @override
  void paint(Canvas canvas, Size size) {
    final paint = Paint()..style = PaintingStyle.stroke..strokeWidth = 2.0..color = color;
    final w = size.width;
    final h = size.height;
    final cx = w / 2;
    final cy = h / 2;
    ...
    switch (batch) {
      ...
      case VisaBatch.global: // 🌍 GLOBE
        ...
        canvas.drawOval(Rect.fromCenter(center: Offset(cx, cy), width: w * 0.7, height: w * 0.2), paint);
        canvas.drawLine(Offset(cx, cy - w*0.35), Offset(cx, cy + w*0.35), paint);
        break;
      ...
      case VisaBatch.oceanic: // Spiral
        final rect = Rect.fromCenter(center: Offset(cx, cy), width: w*0.6, height: h*0.6);
      ...
      case VisaBatch.construct: // Square
        canvas.drawRect(Rect.fromCenter(center: Offset(cx, cy), width: w*0.5, height: w*0.5), paint);
        break;
      default: // Generic Globe
        canvas.drawCircle(Offset(cx, cy), w * 0.3, paint);
        canvas.drawOval(Rect.fromCenter(center: Offset(cx, cy), width: w*0.6, height: h*0.2), paint);
    }
  }
  ...
  void _drawPolygon(Canvas canvas, double cx, double cy, double radius, int sides, Paint paint) {
    final path = Path();
