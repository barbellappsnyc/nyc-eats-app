...
class _BaggageTagBackgroundState extends State<BaggageTagBackground> {
  ...
  void didUpdateWidget(covariant BaggageTagBackground oldWidget) {
    ...
    if (oldWidget.cuisine != widget.cuisine || oldWidget.stamps.length != widget.stamps.length) {
      setState(() {
        _extractDates();
      });
    }
  ...
  Widget build(BuildContext context) {
    ...
    final List<String> destinations = CuisineConstants.airportCodes[cleanCuisine] 
    ...
    return Container(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          String formattedDate = "${tagDate.day.toString().padLeft(2, '0')} ${months[tagDate.month - 1]} ${tagDate.hour.toString().padLeft(2, '0')}:${tagDate.minute.toString().padLeft(2, '0')}";
          ...
          if (destination == 'JFK' || destination == 'EWR' || destination == 'LGA') {
          ...
          final String cityName = CuisineConstants.airportCityNames[destination] ?? '';
...
class SingleBaggageTag extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String englishName = cityParts.length > 1 ? cityParts[1] : cityParts[0];
    ...
    final TextStyle premiumCityStyle = TextStyle(
      fontFamily: testFont, 
      fontSize: 11,
      fontWeight: FontWeight.w600, // Drop to w500 if the Katakana looks too thick
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  Text("FLT: $flightNumber", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 1)),
                  Text(dateStr, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: Colors.black54)),
                ],
              ),
            ),
            
            const Spacer(flex: 3), // 👈 Increased flex pushes the block lower down the tag
            ...
            const Spacer(flex: 2), // 👈 Smaller flex keeps it nested closer to the barcode
