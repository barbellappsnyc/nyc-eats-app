...
class AnimatedCuisinePlaceholder extends StatefulWidget {
  ...
  @override
  State<AnimatedCuisinePlaceholder> createState() =>
      _AnimatedCuisinePlaceholderState();
}

class _AnimatedCuisinePlaceholderState
    extends State<AnimatedCuisinePlaceholder> {
    ...
    return ClipRRect(
      ...
      child: Stack(
        ...
        children: [
          ...
          RepaintBoundary(child: _BakedEmojiOverlay(theme: _theme)),
...
class _BakedEmojiOverlayState extends State<_BakedEmojiOverlay>
  ...
  void _onTick(Duration elapsed) {
    ...
    for (var p in _particles) {
      ...
      if (p.x < -10) {
        p.x = -10;
        p.vx = p.vx.abs();
      } else if (p.x > _currentSize.width - p.width + 10) {
        p.x = _currentSize.width - p.width + 10;
        p.vx = -p.vx.abs();
      }
      ...
      if (p.y < -10) {
        p.y = -10;
        p.vy = p.vy.abs();
      } else if (p.y > _currentSize.height - p.height + 10) {
        p.y = _currentSize.height - p.height + 10;
        p.vy = -p.vy.abs();
      }
    }
    _tickNotifier.value++;
