...
class PillCache {
  static final Map<String, ({int width, int height, Uint8List data})> _memoryCache = {};
  ...
  static Future<({int width, int height, Uint8List data})> getOrGeneratePill(
    String id, String cuisine, String ringType, int stars, bool isDarkMode
  ) async {
    ...
    Color ringColor;
    if (ringType == "gold") ringColor = const Color(0xFFFFD700);
    else if (ringType == "red") ringColor = const Color(0xFFE53935);
    else ringColor = isDarkMode ? const Color(0xFF555555) : const Color(0xFF000000);
  ...
  // =========================================================================
  static Future<({int width, int height, Uint8List data})> _drawPill(String emoji, int stars, Color ringColor, Color fillColor) async {
    ...
    final RRect outerRect = RRect.fromRectAndRadius(Rect.fromLTWH(0, 0, width, height), Radius.circular(height / 2));
    canvas.drawRRect(outerRect, Paint()..color = ringColor..style = PaintingStyle.fill);

    final double strokeWidth = 6.0; 
    final RRect innerRect = RRect.fromRectAndRadius(
      Rect.fromLTWH(strokeWidth, strokeWidth, width - (strokeWidth * 2), height - (strokeWidth * 2)), 
      Radius.circular((height - (strokeWidth * 2)) / 2)
    );
    canvas.drawRRect(innerRect, Paint()..color = fillColor..style = PaintingStyle.fill);

    final bool useDarkText = fillColor.computeLuminance() < 0.5;

    final TextSpan span = TextSpan(
      children: [
        TextSpan(text: emoji, style: const TextStyle(fontSize: 32, fontFamily: 'Apple Color Emoji')), 
        if (stars > 0) const TextSpan(text: "   "),
        if (stars > 0) TextSpan(text: "$stars", style: TextStyle(fontSize: 26, fontWeight: FontWeight.bold, color: useDarkText ? Colors.white : Colors.black)),
        if (stars > 0) const TextSpan(text: "★", style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.red)), 
      ]
    );
    ...
    final Offset textOffset = Offset((width - tp.width) / 2, (height - tp.height) / 2);
...
return (width: width.toInt(), height: height.toInt(), data: byteData!.buffer.asUint8List());
  }
}