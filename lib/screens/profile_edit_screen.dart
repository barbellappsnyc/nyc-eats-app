...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  bool _isLoadingPassports =
      true; // 👈 Starts true so we don't flash "No passports"
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      if (_imageFile != null) {
        ...
        final fileName =
            'guest_profile_${DateTime.now().millisecondsSinceEpoch}.jpg';
      ...
      if (user == null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Profile Saved (Guest Mode)")),
          );
        ...
        if (_imageFile != null) {
          ...
          if (_displayedPhotoUrl != null &&
              _displayedPhotoUrl!.contains('avatars')) {
            try {
  ...
  bool get _hasChanges {
    final currentGender = _selectedGender == 'CUSTOM'
        ? _customGenderController.text.trim()
        : _selectedGender;
      ...
      if (user != null) {
        ...
        try {
          ...
          if (filesToDelete.isNotEmpty) {
            ...
            debugPrint(
              "🧹 Sweeper successfully deleted ${filesToDelete.length} files.",
            );
          }
        } catch (e) {
        ...
        try {
          ...
          if (mounted) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text("Error: Could not delete data. $e")),
            );
  ...
  Future<void> _handleAuthAction() async {
    ...
    if (user == null) {
      ...
      await Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => const AuthScreen(isRedirectingBack: true),
        ),
      );
...
Widget build(BuildContext context) {
  ...
  Widget _buildAuthPill({required bool isGuest}) {
    ...
    const textColor =
        Colors.white; // 👈 Forced to white for both Log In and Log Out
                  ...
                  child: FittedBox(
                    ...
                    child: Text(
                      ...
                      style: TextStyle(
                        ...
                        fontWeight: FontWeight.bold,
                        letterSpacing:
                            0.5, // Tighter letter spacing to fit the long word
                        fontSize: 12,
                      ),
                    ),
                  ),
