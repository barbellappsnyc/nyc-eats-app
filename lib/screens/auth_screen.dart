...
class _AuthScreenState extends State<AuthScreen> {
        ...
        Navigator.pushReplacement(
          ...
          MaterialPageRoute(
            builder: (context) => PassportCollectionScreen(
              initialBookId: (widget.purchasedSku != null)
                  ? 'newly_created_book'
                  : null,
              ...
              TextButton(
                onPressed: isDialogLoading
                    ...
                    : () {
                        ...
                        setState(
                          () => _isLogin = true,
                        ); // Switch UI to login so they can verify later if they want
                      },
                child: const Text(
                  "Cancel",
                    ...
                    : () async {
                        ...
                        try {
                          ...
                          if (context.mounted && response.user != null) {
                            ...
                            setState(
                              () => _isLoading = true,
                            ); // Turn on main loading curtain
                            await _finalizeLogin(
                              response.user!.id,
                            ); // 🟢 Proceed to passports!
                          }
                        } on AuthException catch (e) {
  ...
  void _showForgotPasswordDialog() {
    ...
    final isValidEmail =
        initialEmail.contains('@') && initialEmail.contains('.');

    final resetEmailController = TextEditingController(
      text: isValidEmail ? initialEmail : '',
    );
    final otpController = TextEditingController();
              ...
              child: SingleChildScrollView(
                ...
                child: Row(
                  children: List.generate(12, (colIndex) {
                    final emoji =
                        foodEmojis[(rowIndex + colIndex) % foodEmojis.length];
                    return Padding(
                      padding: const EdgeInsets.all(
                        16.0,
                      ), // Slightly more spacing for bigger emojis
                      child: Text(
                        emoji,
                        style: const TextStyle(
                          fontSize: 36,
                        ), // 👈 Increased size from 22 to 36!
                      ),
                    );
                  }),
                ),
  ...
  Widget build(BuildContext context) {
    ...
    return AnnotatedRegion<SystemUiOverlayStyle>(
      ...
      child: Scaffold(
        backgroundColor:
            Colors.transparent, // 👈 CHANGED: Let the layers show through!
        body: Stack(
          fit: StackFit.expand,
          children: [
            // 🚦 LAYER 1: Plum Red & White Moving Gradient
            const Positioned.fill(child: AnimatedBackground(sku: 'auth')),

            // 🍽️ LAYER 2: The Static Emoji Grid
            Positioned.fill(child: _buildEmojiGrid()),
            ...
            SingleChildScrollView(
              padding: EdgeInsets.only(
                ...
                left:
                    24, // Slightly reduced padding to give the card breathing room
                right: 24,
                bottom: 24,
              ),
              child: ClipRRect(
  ...
  InputDecoration _inputDecoration(String label, IconData icon) {
    return InputDecoration(
      ...
      fillColor: Colors.black.withOpacity(
        0.02,
      ), // 👈 Restored to a subtle, solid grey tint
