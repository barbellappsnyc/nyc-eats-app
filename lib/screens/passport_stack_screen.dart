...
class _PassportStackScreenState extends State<PassportStackScreen>
  ...
  bool _hasCalledReady =
      false; // 👈 NEW: Ensures we only tell the tutorial we are ready once
  ...
  final AudioPlayer _flipPlayer = AudioPlayer()
    ..setPlayerMode(PlayerMode.lowLatency); // 👈 NEW: For physics sounds
  ...
  bool _hasConsumedIncoming =
      false; // 👈 NEW: Tracks if we've already handled the payload
  ...
  void _applyBookData(
    ...
    try {
      ...
      final visasList = List<Map<String, dynamic>>.from(
        bookData['visas'] ?? [],
      );
      ...
      int newActiveIndex = _isLoading
          ? (bookData['last_page_index'] ?? 0)
          : _activeIndex;
      ...
      final List<Map<String, String>> cleanStamps =
          List<Map<String, String>>.from(
            rawStamps.map((item) {
              final name = item['restaurant_name'] ?? item['name'] ?? "Unknown";
              final cuisine =
                  item['country_cuisine'] ?? item['cuisine'] ?? "Global";
  ...
  Future<void> _handleIncomingStampTrigger() async {
    ...
    final bool isDuplicate = collectedStamps.any(
      (s) => (s['name'] ?? s['restaurant_name']) == restaurant.name,
    );
  ...
  Future<void> _initiateImmigrationProtocol() async {
    ...
    try {
      ...
      if (mappingResponse != null)
        ...
        final visaCheck = await Supabase.instance.client
            .from('visa_types')
            .select('cuisine')
            .ilike('cuisine', primaryTag)
            .maybeSingle();
        if (visaCheck != null) targetCuisine = visaCheck['cuisine'];
      }
    } catch (e) {
      debugPrint("Cuisine analysis error: $e");
    }
    ...
    if (widget.incomingRestaurant != null) {
      final bool isDuplicate = PassportBrain.instance.hasDuplicateStamp(
    ...
    final decision = await PassportBrain.instance.processStampRequest(
    ...
    if (decision.action == BrainAction.upgrade) {
      if (_passportSku == 'free_tier') {
        ...
        showDialog(
          context: context,
          builder: (_) => const PassportFullDialog(),
        );
        return;
      }

      final reason = decision.reason ?? "No available space.";
      bool wantsUpgrade = await _showDynamicUpgradeDialog(
        targetCuisine,
        reason,
      );
      ...
      if (wantsUpgrade) {
        if (mounted) {
          await Navigator.push(
            ...
            MaterialPageRoute(
              builder: (_) =>
                  PaywallScreen(incomingRestaurant: widget.incomingRestaurant),
            ),
          );
    ...
    if (decision.action == BrainAction.switchAndStamp) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Switching passport..."),
    ...
    if (decision.action == BrainAction.stayAndStamp) {
      ...
      if (decision.requiresNewVisa && userId != null) {
        ...
        if (!hasCuisine) {
          createNew = await _showVisaApplicationDialog(
            targetCuisine,
            widget.incomingRestaurant!.name,
          );
        } else {
      ...
      final currentBook = library.firstWhere(
        (b) => b['id'] == widget.bookId,
        orElse: () => {},
      );
      int targetPageIndex = PassportBrain.instance.calculateTargetPageIndex(
        currentBook,
        targetCuisine,
      );
      ...
      ); // Tiny pause to let Flutter paint the book
  ...
  Future<void> _executeStampSequence({Restaurant? manualRestaurant}) async {
    ...
    final Restaurant? restaurantToStamp =
        manualRestaurant ?? widget.incomingRestaurant;
    ...
    int currentStampCount = collectedStamps
        .where((s) => s['cuisine'] == targetCuisine)
        .length;
    ...
    final newStamp = {
      ...
      'cuisine': targetCuisine,
      'mta_station_id':
          '', // 👈 NEW: Add an empty string so the UI doesn't crash before the DB syncs
    };

    // This handles the decision: Guest -> Disk, User -> Cloud
    await PassportService.addStamp(bookId: widget.bookId, stampData: newStamp);
  ...
  void _onVerticalDragEnd(DragEndDetails details) {
    ...
    else if (currentDrag > 50 || velocity > 200) {
      // Highly sensitive for simulator
      if (_activeIndex > 0) {
  ...
  void _animateCardAway() {
    ...
    _slideController.forward(from: 0).then((_) {
      ...
      if (widget.bookId != null && !widget.isDemo) {
        // 👈 Prevent demo DB writes
  ...
  // DIALOGS
  Future<bool> _showVisaApplicationDialog(
    String cuisine,
  ...
  Future<bool> _showDynamicUpgradeDialog(
    ...
    String message =
        "You have no available pages for a $targetCuisine visa in your current library.\n\nExpand your collection to continue stamping!";
    IconData icon = Icons.library_books;
    Color iconColor = Colors.blue[900]!;

    if (reason.toLowerCase().contains("single visa") ||
        reason.toLowerCase().contains("monogamy")) {
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () =>
                          Navigator.pop(context, true), // TRUE = Upgrade
                      child: const Text(
                        "UPGRADE PASSPORT",
                        style: TextStyle(
                          fontWeight: FontWeight.bold,
                  ...
                  TextButton(
                    onPressed: () =>
                        Navigator.pop(context, false), // FALSE = Cancel
                    child: const Text(
                      "CANCEL",
  ...
  void _onCardTap() {
    ...
    if (_activeIndex == 0 &&
        (_passportSku == 'diplomat_book' || _passportSku == 'standard_book')) {
      // ... rest of code
      return;
    }
    ...
    if (isAssigned) {
      if (_activeIndex == 0) {
        ...
        if (_passportSku == 'single_page' && userVisas.isNotEmpty) {
          ...
          if (visa['visa_types'] != null &&
              visa['visa_types']['color_hex'] != null) {
            color = _parseColor(visa['visa_types']['color_hex']);
          }

          // C. Use Date
          if (visa['created_at'] != null) {
            dateIssued = DateFormat(
              'MMM d, yyyy',
            ).format(DateTime.parse(visa['created_at']));
          }
        ...
        if (userVisas.length > _activeIndex - 1) {
          ...
          final allCuisineStamps = collectedStamps
              .where((s) => s['cuisine'] == visa['cuisine'])
              .toList();
  ...
  Widget _buildCardContent(
    ...
    if (!ctx.isGlobalPage && !ctx.isVacant) {
      final matchingVisa = userVisas.firstWhere(
        (v) =>
            v['cuisine'].toString().toLowerCase() ==
            ctx.targetCuisine.toLowerCase(),
        orElse: () => {},
      );
      if (matchingVisa.isNotEmpty) {
        if (matchingVisa['visa_types'] != null &&
            matchingVisa['visa_types']['color_hex'] != null) {
          color = _parseColor(matchingVisa['visa_types']['color_hex']);
        }
        if (matchingVisa['created_at'] != null) {
          dateIssued = DateFormat(
            'MMM d, yyyy',
          ).format(DateTime.parse(matchingVisa['created_at']));
        }
      }
    }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                const Text(
                  "This Passport is currently in storage.\n\nTo add a stamp, you must first finish your Primary Passport or activate this one manually.",
                  textAlign: TextAlign.center,
                  style: TextStyle(
                    color: Colors.black54,
                    height: 1.5,
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  const Text(
                    "Immigration Services is currently offline.\n\nPlease connect to the internet to verify and issue new stamps.",
                    textAlign: TextAlign.center,
                    style: TextStyle(
                      color: Colors.black54,
                      height: 1.5,
        ...
        child: Stack(
          ...
          children: [
            ...
            if (_activeIndex < _totalCards)
              GestureDetector(
                onVerticalDragUpdate: _showStampButton
                    ? null
                ...
                child: ValueListenableBuilder<double>(
                  ...
                  child: RepaintBoundary(
                    ...
                    child: SizedBox(
                      ...
                      child: Hero(
                        tag:
                            'passport_card_${widget.bookId}_${_activeIndex + 1}',
                  ...
                  builder: (context, dragValue, cachedCard) {
                    return AnimatedBuilder(
                      ...
                      builder: (context, child) {
                        double currentY = _slideController.isAnimating
                            ? _slideAnim.value
                            : dragValue;
              ...
              Positioned(
                ...
                top: widget.isDemo
                    ...
                    : MediaQuery.of(context).size.height *
                          0.12, // Normal mode uses absolute screen math
